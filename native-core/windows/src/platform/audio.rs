...
// SAFETY: Windows hotkey recording owns start/stop on its action worker thread;
// the mutex only protects access to that worker-owned stream.
...
pub(crate) fn probe_input_device(selected_uid: Option<&str>) -> anyhow::Result<()> {
    let entry = select_input_device(selected_uid)?;
    entry
        .device
...
impl AudioManager {
    ...
    pub fn start_recording(&self, selected_uid: Option<&str>) -> anyhow::Result<()> {
        ...
        let stream = match sample_format {
            ...
            other => {
                return Err(anyhow!(
                    "unsupported Windows microphone sample format: {other:?}"
                ))
            }
        }
    ...
    pub fn stop_recording(&self) -> anyhow::Result<RecordedAudio> {
        ...
        let raw_samples = active
            ...
            .clone();
        let samples_16khz = resample_linear_to_16khz(&raw_samples, active.input_sample_rate);
        let duration_seconds = if samples_16khz.is_empty() {
            active.started_at.elapsed().as_secs_f64()
        } else {
            samples_16khz.len() as f64 / TARGET_SAMPLE_RATE_HZ as f64
        };
    ...
    F: Fn(T) -> f32 + Send + Copy + 'static,
{
    ...
    device
        ...
        .context("failed to build Windows microphone stream")
}
...
fn microphone_capture_error(
    ...
    if is_microphone_privacy_denial(&message) {
        return anyhow!(
            "Windows microphone access is blocked. Enable Microphone access and Let desktop apps access your microphone in Windows Privacy settings, then try again."
        );
    }

    anyhow!("Could not {action} Windows microphone `{device_name}`: {message}")
}

fn microphone_stream_error_message(device_name: &str, error: &str) -> String {
    if is_microphone_privacy_denial(error) {
        return "Windows microphone access was blocked while recording. Enable Microphone access and Let desktop apps access your microphone in Windows Privacy settings, then try again.".into();
    }

    format!(
        "Windows microphone `{device_name}` stopped while recording. Check that the microphone is still connected and selected, then try again. Details: {error}"
    )
}
...
fn input_device_entries() -> anyhow::Result<Vec<InputDeviceEntry>> {
    ...
    for device in host
        .input_devices()
        .context("failed to enumerate Windows input devices")?
    {
        let name = device
            .name()
...
fn select_input_device(selected_uid: Option<&str>) -> anyhow::Result<InputDeviceEntry> {
    let entries = input_device_entries()?;
    if entries.is_empty() {
        return Err(anyhow!("No Windows microphone input devices were found."));
    }
...
mod tests {
    ...
    #[test]
    fn selected_uid_exists_checks_windows_device_list() {
    ...
    #[test]
    fn microphone_privacy_errors_include_windows_settings_guidance() {
        let error = microphone_capture_error("start", "USB Mic", "access denied").to_string();

        assert!(error.contains("Windows microphone access is blocked"));
        assert!(error.contains("Let desktop apps access your microphone"));
    }

    #[test]
    fn microphone_stream_errors_report_device_disconnect_context() {
        let error = microphone_stream_error_message("USB Mic", "device removed");

        assert!(error.contains("USB Mic"));
        assert!(error.contains("stopped while recording"));
        assert!(error.contains("still connected"));
    }
}
